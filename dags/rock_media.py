...
def fetch_and_save_media(ds, *args, **kwargs):
    ...
    def mapContentItems(contentItem):
        ...
        def filter_media_attributes(attribute):
            return (
                is_media_image(attribute)
                or is_media_video(attribute)
                or is_media_audio(attribute)
            )
        ...
        def get_media_value(attribute):
            ...
            if mediaType == "IMAGE":
                return (
                    "https://rock.apollos.app/GetImage.ashx?guid=" + attributeValue
        ...
        filteredAttributes = filter(
            filter_media_attributes, contentItem["Attributes"].values()
        )
