...
from wista import set_wistia_urls
import json
    ...
    def is_media_image(attribute, contentItem):
        ...
        if attribute['FieldTypeId'] == 133:
            try:
                ...
                return (
                    'audio' not in parsed_value['Key'].split('/') and
                    'video' not in parsed_value['Key'].split('/') and
                    '.mp3' not in parsed_value['Key']
                )
...
globals()['rock_content_item_backfill_newspring_dag'] = create_content_item_dag(
    'rock_content_item_backfill_newspring_dag',
    start_date,
    '@once',
    True
)

globals()['rock_content_item_newspring_dag'] = create_content_item_dag(
    'rock_content_item_newspring_dag',
    start_date,
