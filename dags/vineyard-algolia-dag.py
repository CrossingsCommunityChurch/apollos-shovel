...
def algolia():
    ...
    categories = pg_hook.get_records(
        "select id from content_item_category where title in ('Go Do Good: Service Opportunities')"
    )
    ...
    client = SearchClient.create("BBWBK2FDR6", Variable.get("vineyard_algolia_key"))
...
dag, dag_name = create_algolia_dag("vineyard", start_date, timedelta(hours=12))
