...
def algolia():
    ...
    pg_hook = PostgresHook(
        postgres_conn_id="crossings_apollos_postgres",
        keepalives=1,
        keepalives_idle=30,
    ...
    categories = pg_hook.get_records(
        "select id from content_item_category where origin_id in ('21','22','423','23')"
    )
    ...
    client = SearchClient.create("50RBZ7CSQI", Variable.get("crossings_algolia_key"))
...
dag, dag_name = create_algolia_dag("crossings", start_date, timedelta(hours=12))
